...
CREATE TABLE x_items (
  item_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- int型 自動採番 主キー
  name VARCHAR(32) NOT NULL UNIQUE, -- 可変長文字型 非NULL制約 一意制約
  price INTEGER NOT NULL CHECK (price >= 0), -- int型 非NULL制約 CHECK制約(値段が0以上)
  weight_kg DECIMAL(4, 1) NOT NULL, -- 固定小数点型(整数3桁+少数1桁) 非NULL制約
  description TEXT -- TEXT型
);
...
CREATE TABLE x_characters (
  ...
  level INTEGER NOT NULL CHECK (level BETWEEN 1 AND 255), -- int型 非NULL制約 CHECK制約(1と255の間の値 1と255も含む)
  job_id INTEGER NOT NULL REFERENCES x_jobs (job_id), -- int型 非NULL制約 REFERENCES(外部主キー x_jobsテーブルのjob_idを参照)
  created_at TIMESTAMP NOT NULL DEFAULT LOCALTIMESTAMP(0), -- TIMESTAMP型 非NULL制約 初期値としてタイムゾーンなし，小数点以下なしの日時を設定
  deleted_at TIMESTAMP,
  CHECK (
    deleted_at IS NULL OR
    deleted_at >= created_at
  ) -- TIMESTAMP型 CHECK制約(NULLかcreated_atより大きいこと)
);
...
CREATE TABLE x_guilds (
  ...
  name VARCHAR(16) NOT NULL UNIQUE,
  owner_id INTEGER NOT NULL REFERENCES x_characters (character_id) ON DELETE CASCADE -- int型 非NULL制約 外部主キー(x_charactersテーブルのcharacter_id) x_charactersテーブルのレコードが削除されたら同じく削除
);
...
CREATE TABLE x_guild_characters (
  ...
  PRIMARY KEY (character_id, guild_id) -- 複合主キー
);
